...
async function fetchCompilerModule(version: string): Promise<CompilerModule>
{
    try {
        ...
        console.log({ compilerModuleURL })
        const compilerModuleNamespace = (await import(/* @vite-ignore */compilerModuleURL)) as CompilerModule
        console.log({ compilerModuleNamespace })
...
export async function fetchLatestProductionCompilerVersion(): Promise<string>
{
    ...
    const { productionVersions: productionCompilerVersions } = getCompilerVersionsByType(allCompilerVersions)
...
export async function fetchCompilerModuleAndWASM(version: string): Promise<CompilerModuleAndWasm | null>
{
    ...
    console.log("Computed compiler wasm url:")
    console.log({ compilerWasmUrl })
    console.log("Computed compiler module namespace:")
    console.log({ compilerModuleNamespace })
    if (!compilerModuleNamespace || !compilerWasmUrl) {
        console
        console.warn(`Failed to fetch compiler module or wasm for version ${version}`)
        return null
    }
